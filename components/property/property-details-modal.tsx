...
export function PropertyDetailsModal({
  ...
  const propertiesWithImageUrls = {
    ...property,
    images:
      property.images?.map((imageId: any) => `${IMAGE_BASE_URL}${imageId}`) ||
      [],
  };
  console.log(propertiesWithImageUrls);
  const { images, name, status } = propertiesWithImageUrls;
                ...
                <div className="flex justify-between items-center mb-4">
                  ...
                  <div className="flex items-center gap-2">
                    {status === "pending" && (
                      <>
                        <button
                          onClick={() => onEdit?.(propertiesWithImageUrls.id)}
                          className="p-2 hover:bg-gray-100 rounded-full"
                        >
                          <Pencil className="h-5 w-5 text-gray-500" />
                        </button>
                        <button
                          onClick={() => onDelete?.(propertiesWithImageUrls.id)}
                          className="p-2 hover:bg-gray-100 rounded-full"
                        >
